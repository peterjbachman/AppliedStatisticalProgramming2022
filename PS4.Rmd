...
top_retweets <- tweets_retweets[1:500, ]

Corpus <- VCorpus(VectorSource(top_retweets$text))
...
## using the max.words argument causes issues with the colors argument since the
## dataset is longer than the vector.
wordcloud(words_freq$Word[1:50],
  words_freq$Frequency[1:50],
  random.order = TRUE,
  ordered.colors = TRUE,
  colors = colors
)
